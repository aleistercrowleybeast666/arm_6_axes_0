var ArmRevA = (() => {
  ...
  __export(generator_exports, {
    ...
    Component_WirePins: () => Component_WirePins,
    Design_CheckPinName: () => Design_CheckPinName,
    Design_Generate: () => Design_Generate,
    Design_NormalizeName: () => Design_NormalizeName,
    Footprint_ReadPads: () => Footprint_ReadPads,
    Library_ResolvePart: () => Library_ResolvePart,
    Project_Create: () => Project_Create,
    Project_Export: () => Project_Export,
    Project_PrepareSchematic: () => Project_PrepareSchematic,
    Review_ExportSheet: () => Review_ExportSheet,
    Sheet_AddNotes: () => Sheet_AddNotes,
    Sheet_AuditAndSave: () => Sheet_AuditAndSave,
    Sheet_CheckERC: () => Sheet_CheckERC,
  ...
  async function Library_ResolvePart(eda, part) {
    ...
    for (const item of candidates || []) {
      ...
      try {
        detail = await eda.lib_Device.get(item.uuid, libraryUuid);
      } catch (error) {
  ...
  async function Project_Create(eda, design) {
    ...
    if ((await eda.dmt_Project.getCurrentProjectInfo())?.uuid !== projectUuid && !await eda.dmt_Project.openProject(projectUuid)) throw new Error(`Project ${projectUuid} created but openProject failed. Inspect it before any retry.`);
    return await Project_PrepareSchematic(eda, design, projectUuid);
  }
  async function Project_PrepareSchematic(eda, design, projectUuid) {
  ...
  async function Sheet_Open(eda, project, sheet) {
    const active = await eda.dmt_Project.getCurrentProjectInfo();
  ...
  async function Sheet_AddNotes(eda, design, sheet) {
  ...
  async function Component_Place(eda, component, part, device) {
    ...
    const changed = await eda.sch_PrimitiveComponent.modify(id, {
  ...
  async function Component_WirePins(eda, placed, offset = 0, count = 10) {
  ...
  async function Sheet_AuditAndSave(eda, placed) {
  ...
  async function Sheet_CheckERC(eda) {
  ...
  async function Project_Export(eda, outputPath) {
    const file = await eda.sys_FileManager.getProjectFile("arm_6_axes_0_revA.epro", void 0, "epro");
    if (!file) throw new Error("Editable project export failed.");
    if (outputPath) {
      if (!await eda.sys_FileSystem.saveFileToFileSystem(outputPath, file, void 0, false)) throw new Error("Export path exists or is not writable; no file overwritten.");
    } else await eda.sys_FileSystem.saveFile(file, "arm_6_axes_0_revA.epro");
    return { editableProjectExported: true, size: file.size };
  }
  async function Review_ExportSheet(eda, outputDirectory, sheet) {
    ...
    for (const [file, suffix] of [[svg, ".svg"], [netlist, ".net"]]) {
      if (!await eda.sys_FileSystem.saveFileToFileSystem(outputDirectory + "/" + sheet + suffix, file, void 0, false)) throw new Error("Review export failed or file exists.");
    }
    return { svg: true, netlist: true };
  }
  async function Design_Generate(eda, design) {
    const devices = {};
  ...
  return __toCommonJS(generator_exports);
})();
